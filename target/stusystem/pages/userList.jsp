    ...
    <div class="main-content">
        ...
        <ul class="user-info-menu right-links list-inline list-unstyled">
            <li class="search-form">
                <s:form id="seachFrm" action="userAction_selectAllUser" method="post">
                    用户名：<s:textfield name="querys" size="10" value="%{querys[0]}"/>-
                    用户电话:<s:textfield name="querys" size="10" value="%{querys[1]}"/>-
                    用户邮箱:<s:textfield name="querys" size="10" value="%{querys[2]}"/>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <s:iterator var="obj" value="pageBean.result" status="ids">
                    <tr>
                        <td>${(pageBean.cpage-1)*pageBean.showNum+ids.count}</td><td>${obj.uname}</td>
                        <td>${obj.uphone}</td><td>${obj.uemail}</td>
                        <td><img src="${pageContext.request.contextPath}/${obj.uploadImage}" style="width: 30px; height: 35px"></td><td>${(obj.utype==1?"业务人员":"研发人员")}</td>
                        <td>${obj.uintime.toString().substring(0,10)}</td><td>${obj.ustatus==0?"无效":"有效"}</td>
                        <td>${obj.uremark}</td>
                        <td><button type="button" class="btn btn-success" onclick="window.location.href='userAction_selectByIdUsers?user.uno=${obj.uno}'">更新</button><br/>
                        <td><button type="button" class="btn btn-danger" onclick="window.location.href='userAction_deleteUsers?user.uno=${obj.uno}'">删除</button><br/>
                    </tr>
                </s:iterator>
                </tbody>
            </table>
    ...
    <%-- 查询所有客户信息 --%>
        <div style="position: relative;margin-top: 20px;float: right">
            ...
            <c:forEach var="i" begin="1" end="${pageBean.allPage }" step="1">
                <input type="button" value="第${i }页" onClick="location.href='userAction_selectAllUser?pageBean.cpage=${i}'"/>
            </c:forEach>
            <a href="userAction_selectAllUser?pageBean.cpage=1">首页</a>|
            <a href="userAction_selectAllUser?pageBean.cpage=${pageBean.cpage-1>0?pageBean.cpage-1:1}">上一页</a>|
            <a href="userAction_selectAllUser?pageBean.cpage=${pageBean.cpage+1>pageBean.allPage?pageBean.allPage:pageBean.cpage+1}">下一页</a>|
            <a href="userAction_selectAllUser?pageBean.cpage=${pageBean.allPage}">尾页</a>
            <form action="userAction_selectAllUser" method="post">
