    ...
    <div class="main-content">
        ...
        <ul class="user-info-menu right-links list-inline list-unstyled">
            <li class="search-form"><!-- You can add "always-visible" to show make the search input visible -->
                ...
                <s:form id="seachFrm" action="customerAction_selectAllCustomer" method="post">
                    客户名称：<s:textfield name="querys" size="10" value="%{querys[0]}"/>-
                    客户税号:<s:textfield name="querys" size="10" value="%{querys[1]}"/>-
                    联系人:<s:textfield name="querys" size="10" value="%{querys[2]}"/>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <s:iterator var="obj" value="pageBean.result" status="ids">
                    <tr>
                        <td>${(pageBean.cpage-1)*pageBean.showNum+ids.count}</td><td>${obj.cname}</td>
                        <td>${obj.ctaxcode}</td><td>${obj.clinkman}</td>
                        <td>${obj.cphone}</td><td>${obj.caddress}</td>
                        <td>${obj.dcreatedatetime.toString().substring(0,10)}</td><td>${obj.cexplain}</td>
                        <td>${obj.cstatus==0?"锁定":"正常"}</td><td>${obj.cremark}</td>
                        <td><button type="button" class="btn btn-success" onclick="window.location.href='customerAction_selectByIdCustomer?customer.cid=${obj.cid}'">更新</button><br/>
                        <td><button type="button" class="btn btn-danger" onclick="window.location.href='customerAction_deleteCustomer?customer.cid=${obj.cid}'">删除</button><br/>
                    </tr>
                </s:iterator>
                </tbody>
            </table>
    ...
    <%-- 查询所有客户信息 --%>
        <div style="position: relative;margin-top: 20px;float: right">
            ...
            <c:forEach var="i" begin="1" end="${pageBean.allPage }" step="1">
                <input type="button" value="第${i }页" onClick="location.href='customerAction_selectAllCustomer?pageBean.cpage=${i}'"/>
            </c:forEach>
            <a href="customerAction_selectAllCustomer?pageBean.cpage=1">首页</a>|
            <a href="customerAction_selectAllCustomer?pageBean.cpage=${pageBean.cpage-1>0?pageBean.cpage-1:1}">上一页</a>|
            <a href="customerAction_selectAllCustomer?pageBean.cpage=${pageBean.cpage+1>pageBean.allPage?pageBean.allPage:pageBean.cpage+1}">下一页</a>|
            <a href="customerAction_selectAllCustomer?pageBean.cpage=${pageBean.allPage}">尾页</a>
            <form action="customerAction_selectAllCustomer" method="post">
