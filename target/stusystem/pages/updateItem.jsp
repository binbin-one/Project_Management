    ...
    <div class="main-content">
        ...
        <div>
            <s:if test="#parameters.msg[0]=='success'">
                <p style="color:#979898">更新成功!</p>
            </s:if>
            <s:elseif test="#parameters.msg[0]=='error'">
                <p style="color:#979898">更新失败!</p>
            </s:elseif>
        </div>

        <form method="post"  action="itemAction_updateItem" onsubmit="return test()">
            <s:hidden name="item.cid"/><br/>
            <div class="form-group">
                <label name="item.cname">项目名称<span id="span1" style="display: none"></span></label>
                <s:textfield name="item.cname" class="form-control" style="width: 300px"/><br/>
            </div>
            选择产品名称:<select name="item.product.cid">
            <option value="">请选择产品</option>
            </select><br/>
            选择产品客户姓名:<select name="item.customer.cid">
            <option value="">请选择客户</option>
        </select><br/>
            选择产品业务人员姓名:<select name="item.users.uno">
            <option value="">请选择业务人员</option>
        </select><br/>
            <script type="text/javascript">
                $(function(){
                    /*异步查询产品信息*/
                    $(":input[name='item.product.cid'] option:not(:first)").remove();
                    $.post("${pageContext.request.contextPath}/itemAction_selectProduct",null,function(data){
                        $.each(data.plist,function(i,v){
                            var option="<option value='"+v.cid+"'>"+v.cname+"</option>";
                            $(":input[name='item.product.cid']").append(option);
                        });
                    },"json");
                /*异步查询客户信息*/
                $(":input[name='item.customer.cid'] option:not(:first)").remove();
                $.post("${pageContext.request.contextPath}/itemAction_selectCustomer",null,function(data){
                    $.each(data.clist,function(i,v){
                        var option="<option value='"+v.cid+"'>"+v.cname+"</option>";
                        $(":input[name='item.customer.cid']").append(option);
                    });
                },"json");
                    /*异步查询业务人员信息*/
                    $("select[name='item.users.uno'] option:not(:first)").remove();
                    $.post("${pageContext.request.contextPath}/itemAction_selectBusiness",null,function(data){
                        $.each(data.slist,function(i,v){
                            var option="<option value='"+v.uno+"'>"+v.uname+"</option>";
                            $("select[name='item.users.uno']").append(option);
                        });
                    },"json");
                });
            </script>
            <div class="form-group">
                <label name="item.cexplain">简介<span id="span" style="display: none"></span></label>
                <s:textfield name="item.cexplain" class="form-control" style="width: 300px"/><br/>
            </div>
            <div class="form-group">
                <label name="item.dsalary">金额<span id="span2" style="display: none"></span></label>
                <s:textfield name="item.dsalary" class="form-control" style="width: 300px"/><br/>
            </div>
            <div class="form-group">
                <label name="item.dcreatedatetime">创建日期<span id="span3" style="display: none"></span></label>
                <s:textfield name="item.dcreatedatetime" class="form-control" style="width: 300px"/><br/>
            </div>
            <label class="control-label" name="item.cusetype" style="color: #979898;">项目类型</label>
            <s:radio list="#{'0':'试用','1':'正式'}" name="item.cusetype"/><br/>
            <label class="control-label" name="item.cpaytype" style="color: #979898;">付款状态</label>
            <s:radio list="#{'0':'未结款','1':'结款中','2':'已结款'}" name="item.cpaytype"/><br/>
            <label class="control-label" name="item.cstatus" style="color: #979898;">项目状态</label>
            <s:radio list="#{'0':'无效','1':'有效'}" name="item.cstatus"/><br/>
            <script type="text/javascript">
                //获取性别单选框
                var cusetype=document.forms[0].item.cusetype;//获取sex单选按钮组
                if(cusetype[0].value=='${item.cusetype}'){
                    cusetype[0].checked=true;
                }else{
                    cusetype[1].checked=true;
                }
                var cpaytype=document.forms[0].item.cpaytype;//获取sex单选按钮组
                if(cpaytype[0].value=='${item.cpaytype}'){
                    cpaytype[0].checked=true;
                }else{
                    cpaytype[1].checked=true;
                }
                var cstatus=document.forms[0].item.cstatus;//获取sex单选按钮组
                if(cstatus[0].value=='${item.cstatus}'){
                    cstatus[0].checked=true;
                }else{
                    cstatus[1].checked=true;
                }
            </script>
            <div class="form-group">
                <label name="item.cremark">备注<span id="span4" style="display: none"></span></label>
                <s:textfield name="item.cremark" class="form-control" style="width: 300px"/><br/>
            </div>
            <button type="submit" class="btn btn-default">更新项目</button>
        </form>
    ...
    <div id="chat" class="fixed">
        ...
        <div class="chat-inner">
            ...
            <script type="text/javascript">
                ...
                function test(){
                    if($("#cname").val()=="") {
                        $("#span1").text('客户名不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }
                    if(!$("#username").val()==""){
                    ...
                    if($("#ctaxcode").val()==""){
                        $("#span2").text('客户税号不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }
                    if(!$("#password").val()==""){
                    ...
                    if($("#clinkman").val()==""){
                        $("#span3").text('联系人不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }

                    if($("#cphone").val()=="") {
                        $("#span4").text('电话不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }
                    if(!$("#email").val()==""){
                        var str_email = document.getElementById("email").value;
                        var patt4= /^(13[0-9]{9})|(15[89][0-9]{8})|(18[0-7]{9})$/;
                    ...
                    if($("#caddress").val()=="") {
                        $("#span5").text('地址不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }

                    if($("#dcreatedatetime").val()=="") {
                        $("#span6").text('日期不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }
                    if(!$("#dcreatedatetime").val()==""){
                        var str_email = document.getElementById("dcreatedatetime").value;
                        var patt4= /^\d{4}-\d{1,2}-\d{1,2}$/;
                        if(!patt4.test(str_email)){
                            alert("日期格式错误！");
                            return false;
                        }
                    ...
                    if($("#uremark").val()==""){
                        $("#span8").text('备注不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }
                    if($("#cexplain").val()==""){
                        $("#span7").text('说明不能为空').css('color','#979898').css('margin-left','260px').css('margin-top','-36px').css('display','block');
                        return false;
                    }
                }
            </script>
